...
export function help() {
  return renderHelp({
    description: "Checks for license compliance of the installed packages.",
    descriptionLists: [
      {
        title: "Options",
...
export async function handler(
  ...
  } & Pick<Config, "ca" | "production" | "dev" | "optional" | "virtualStoreDir">
) {
  ...
  if (lockfile == null) {
    throw new PnpmError(
      ...
      `No ${WANTED_LOCKFILE} found: Cannot license compliance check a project without a lockfile`
    );
  }

  if (!opts.virtualStoreDir) {
    throw new PnpmError(
      "LICENSES_NO_VIRTUAL_STORE_DIRECTORY",
      "No virtual store directory found: Cannot license compliance check a project without a virtual store directory"
    );
  }
